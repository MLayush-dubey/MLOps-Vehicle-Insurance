...
#this class is the data container-->individual input variables leta hai aur usko df meh convert krta h for model pred
class VehicleData:
    def __init__(self,
                Gender,
                Age,
        ...
        """
        #current input ko save krle rahe h for a particular user
        try:
            self.Gender = Gender 
            self.Age = Age 
...
#jo save kiya hua h constructor meh, ussiko humlog dataframe meh convert krde rahe hai
    def get_vehicle_input_data_frame(self) -> DataFrame:
        """
        This function returns a dataframe
        """
        try:
            vehicle_input_dict = self.get_vehicle_data_as_dict()  #niche vale func ko call kr raha hai
            return DataFrame(vehicle_input_dict)
        
        except Exception as e:
            raise MyException(e, sys) from e 
        
#ye main self values ko lekar dictionary meh convert krta h aur fir uppar method ko bhejta h to convert to dataframe
...
#since pandas expects inputs in the form of list/array to convert to dataframe, hence we passed in []
        try:
            ...
            logging.info("Exited the get_vehicle_data_as_dict method as VehicleData class")
            return input_data 
...
#delivery service class--> looks for the model in s3, uses proj1estimator to pull that model        
class VehicleDataClassifier:
    ...
    def predict(self, dataframe) -> str:
        ...
        try:
            ...
            result = model.predict(dataframe)  #proj1estimator class k andhar ka method
