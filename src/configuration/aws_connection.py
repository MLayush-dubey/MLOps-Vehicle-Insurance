...
#establishes connection to AWS s3
class S3Client:
    ...
    def __init__(self, region_name=REGION_NAME):
        """
        This client gets aws credentials from env_variable and creates an connection with s3 bucket
        and raise exception when env variable is not set
        """

        if S3Client.s3_client==None or S3Client.s3_resource==None:
            __access_key_id = os.getenv(AWS_ACCESS_KEY_ID_ENV_KEY, )  #IAM USER CREDENTIALS--> read from environment
